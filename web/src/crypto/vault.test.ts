...
const FAST: KdfParams = DEFAULT_KDF;
...
describe("vault key chain", () => {
  it("walks all three questions in order and opens the vault", async () => {
    const e = await enroll("correct horse", questions, answers, undefined, FAST);
  ...
  it("ignores case, spacing and unicode form, but not answer boundaries", async () => {
    const e = await enroll("pw", questions, answers, undefined, FAST);
  ...
  it("rekeying keeps the vault key, so existing items still open", async () => {
    ...
    const rekeyed = await enroll("new pw", ["a?", "b?", "c?"], ["1", "2", "3"], vaultKey, FAST);
